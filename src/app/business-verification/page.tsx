...
export default function BusinessVerificationPage() {
  ...
  const getVerificationStatus = () => {
    ...
    const allDocs = allTypes
      .map((type) => getDocumentByType(type))
      .filter(Boolean);

    if (allDocs.length === allTypes.length) {
      return {
        status: "Fully Verified",
        message: "All documents have been uploaded successfully",
      };
    } else if (requiredDocs.length === requiredTypes.length) {
      return {
        status: "Limited Verified",
        message:
          "Required documents uploaded. Upload remaining documents for full verification",
      };
              ...
              <div className="border border-gray-100 rounded-xl overflow-hidden bg-white shadow-sm">
                ...
                <div
                  className={`bg-gradient-to-br border rounded-xl p-6 flex flex-col sm:flex-row gap-4 shadow-lg ${
                    getVerificationStatus().status === "Fully Verified"
                      ? "from-green-50 to-emerald-50 border-green-200"
                      : getVerificationStatus().status === "Limited Verified"
